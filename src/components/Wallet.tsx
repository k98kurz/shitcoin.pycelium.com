...
interface WalletProps {
  ...
  currentPrice: number;
  onWalletUpdate: (
    updater: (prevWallet: { $HIT: number; fAuxUSD: number; i$HIT: number }) => {
      $HIT: number;
      fAuxUSD: number;
      i$HIT: number;
    },
...
const Wallet: React.FC<WalletProps> = ({
  ...
  const score = useMemo(() => {
    const totalValue =
      fAuxUSDBalance + $hitBalance * currentPrice + i$hitBalance / currentPrice;
  ...
  const handleConfirm = () => {
    if (modalStage === "confirm") {
      ...
      blockchainTimeoutRef.current = window.setTimeout(() => {
        ...
        onWalletUpdate((prev) => {
          const convertedUSD =
            prev.fAuxUSD + prev.$HIT * currentPrice + prev.i$HIT / currentPrice;
          const profit = convertedUSD - 420.69;
          if (profit > 0) {
            setResultMessage(
              `Congrats, you cleared ${profit.toFixed(2)} fAuxUSD profit`,
            );
          } else {
            setResultMessage(
              `You gave up ${Math.abs(profit).toFixed(2)} fAuxUSD, thanks for your business`,
            );
          }
    ...
    <div className="bg-white p-6 rounded-lg shadow-md relative">
      <h2 className="text-xl font-semibold mb-4 flex items-center text-gray-700">
        <WalletIcon
          ...
          className="mr-2 h-5 w-5 text-indigo-600 cursor-pointer"
        />{" "}
        My Wallet
      </h2>
      <div className="space-y-3">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-lg font-bold text-purple-700">
            {i$hitBalance.toFixed(4)}
          </span>
        </div>
        <div className="flex justify-between items-center">
          <span className="font-medium text-gray-600">Score:</span>
          <span
            className={`text-lg font-bold ${score > 0 ? "text-blue-700" : "text-orange-700"}`}
          >
            {score.toFixed(3)}
          </span>
        </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {modalStage === "result" && (
              <>
                ...
                <p className="text-sm">
                  Redirecting to the next phase in 10 seconds...
                </p>
              </>
            )}
          </div>
